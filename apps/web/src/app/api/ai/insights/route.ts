...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${backendUrl}/api/v1/ai/insights`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      return apiError(
        { success: false, error: { code: data.error?.code || data.code || "AI_ERROR", message: data.message || "AI Insights failed" } },
        res.status
      );
    }
