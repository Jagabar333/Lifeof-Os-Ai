...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${backendUrl}/api/v1/ai/notes/summarize`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      return apiError({ success: false, error: { code: "AI_ERROR", message: data.message || "AI summarization failed" } });
    }
