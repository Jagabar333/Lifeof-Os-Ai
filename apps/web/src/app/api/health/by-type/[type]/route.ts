...
export async function GET(req: NextRequest, { params }: { params: Promise<{ type: string }> }) {
  try {
    ...
    const res = await fetch(`${backendUrl}/api/v1/health/by-type/${type}?days=${days}`, {
      headers: {
    ...
    if (!res.ok) {
      return apiError({ success: false, error: { code: "DB_ERROR", message: data.message || "Failed to fetch health metrics by type" } });
    }
